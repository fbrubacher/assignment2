...
class Ufc(DataLoader):
    def __init__(self, path='data/preprocessed_data.csv', verbose=True, seed=1):
    ...
    def pre_training_adjustment(self, train_features, train_classes):
        ...
        :return: The processed data
        """
        sp = SparseRandomProjection(eps=0.3, n_components=160)
        train_features = sp.fit_transform(train_features)
        return train_features, train_classes
...
class AdultEarnings(DataLoader):
    def __init__(self, path='data/adult.csv', verbose=True, seed=1):
        super().__init__(path, verbose, seed)
    ...
    def _preprocess_data(self):
        # https://www.ritchieng.com/machinelearning-one-hot-encoding/
        to_encode = ['income', 'native.country', 'sex', 'race', 'relationship', 'occupation', 'marital.status','education', 'workclass']
        label_encoder = preprocessing.LabelEncoder()
        one_hot = preprocessing.OneHotEncoder()
